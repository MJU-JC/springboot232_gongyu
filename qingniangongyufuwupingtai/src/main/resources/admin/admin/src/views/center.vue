  ...
  <div>
    ...
    >  
     <el-row>
         ...
         <el-col :span="12">
             <el-form-item v-if="flag=='yonghu'"  label='账户状态' prop="yonghuShiyongTypes">
                 <el-select v-model="ruleForm.yonghuShiyongTypes" disabled  placeholder='请选择账户状态'>
                     <el-option
                             v-for="(item,index) in yonghuShiyongTypesOptions"
         ...
         <el-col :span="12">
             <el-form-item v-if="flag=='shangjia'" label='商家照片' prop="shangjiaPhoto">
                 <file-upload
                         tip="点击上传照片"
                         action="file/upload"
                         :limit="3"
         ...
         <el-col :span="12">
             <el-form-item v-if="flag=='shangjia'"  label='账户状态' prop="shangjiaShiyongTypes">
                 <el-select v-model="ruleForm.shangjiaShiyongTypes" disabled  placeholder='请选择账户状态'>
                     <el-option
                             v-for="(item,index) in shangjiaShiyongTypesOptions"
                             v-bind:key="item.codeIndex"
                             :label="item.indexName"
                             :value="item.codeIndex">
                     </el-option>
...
// 注册表 用户
    // 注册的类型字段 用户
        ...
        sexTypesOptions : [],
        // 账户状态
        yonghuShiyongTypesOptions : [],
// 注册表 商家
    // 注册的类型字段 商家
        // 商家信用类型
        shangjiaXingjiTypesOptions : [],
        // 账户状态
        shangjiaShiyongTypesOptions : [],
    };
  },
  mounted() {
    //获取当前登录用户的信息
...
// 注册表 商家 的级联表
   ...
   this.$http({
       url: `dictionary/page?page=1&limit=100&sort=&order=&dicCode=yonghu_shiyong_types`,
       method: "get"
   }).then(({ data }) => {
       if (data && data.code === 0) {
          this.yonghuShiyongTypesOptions = data.data.list;
      } else {
          this.$message.error(data.msg);
      }
    });
   ...
   this.$http({
       url: `dictionary/page?page=1&limit=100&sort=&order=&dicCode=shangjia_shiyong_types`,
       method: "get"
   }).then(({ data }) => {
       if (data && data.code === 0) {
          this.shangjiaShiyongTypesOptions = data.data.list;
      } else {
          this.$message.error(data.msg);
      }
    });
  ...
  methods: {
    ...
    onUpdateHandler() {
                         ...
                         if((!this.ruleForm.yonghuShiyongTypes)&& 'yonghu'==this.flag){
                         ...
                         if((!this.ruleForm.shangjiaShiyongTypes)&& 'shangjia'==this.flag){
                             this.$message.error('账户状态不能为空');
                             return
                         }
